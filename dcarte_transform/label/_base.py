...
def _label(
    ...
    assert id_col in df.columns, (
        "Please ensure that the id_col is in the data frame. "
        "You should specify id_col=[THE ID COLUMN]"
    )
    assert datetime_col in df.columns, (
        "Please ensure that the datetime_col is in the data frame. "
    ...
    logging.info(
        "Making sure the pandas columns that are used for the join have the same type."
    )
    ...
    df_labelled = (
        pd.merge(df, df_labels, how="left", on=[id_col, "__date__"])
        .copy()
        .drop("__date__", axis=1)
        .copy()
    )
...
def _label_number_previous(
    ...
    cumulative_pos_labels = (
        ...
        .reset_index()
        .sort_values(by="date")
        .rename({"outcome": f"previous_outcome", "date": "__date__"}, axis=1)
