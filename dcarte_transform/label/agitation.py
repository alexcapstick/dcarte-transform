...
def resample_labels_database(database: pd.DataFrame, most_frequent_ids: pd.DataFrame):
    """
    ...
    for patient in most_frequent_ids["patient_id"]:
        current_data = database[database["patient_id"] == patient].reset_index(
            drop=True
        )
        if not current_data.empty:
            current_data = current_data.drop_duplicates(subset="start_date").set_index(
                "start_date"
            )
            new_index = pd.date_range(
                current_data.index.min(), current_data.index.max(), freq="D"
            )
...
def get_labels(
    days_either_side: int = 0, return_event: bool = False, all_symptoms: bool = False
) -> pd.DataFrame:
    """
    ...
    minder_df.loc[
        minder_df["notes"].str.contains("positive", na=False), "label"
    ] = "positive"
    minder_df.loc[
        minder_df["notes"].str.contains("negative", na=False), "label"
    ] = "negative"
    ...
    minder_df = minder_df.sort_values("label", ascending=False).drop_duplicates(
        subset=["patient_id", "start_date"]
    )
    minder_df = minder_df.sort_values(["patient_id", "start_date"]).reset_index(
        drop=True
    )
    ...
    if all_symptoms:
        questions = [
            "weekly-checking-PLWDAgitation-boolean",
    ...
    questionnaire = questionnaire[
        questionnaire["question"].isin(questions)
    ].reset_index(drop=True)
    questionnaire["question"].replace(
        {
            "weekly-checking-PLWDAgitation-boolean": "Neuropsychiatric",
            "weekly-checking-PLWDConfusion-boolean": "Neuropsychiatric",
        },
        inplace=True,
    )
    ...
    questionnaire.rename(
        columns={"question": "type", "answer": "label", "authored": "start_date"},
        inplace=True,
    )
    ...
    tihm_df = tihm_df.rename(
        columns={
            "datetimeRaised": "start_date",
            "subjectdf": "patient_id",
            "valid": "label",
        }
    )
    ...
    df_labels = (
        pd.concat([tihm_df, minder_df])
        .drop(columns="type")
        .drop_duplicates()
        .reset_index(drop=True)
    )
    df_labels = df_labels.rename(
        columns={"start_date": "date", "label": "agitation_label"}
    )
    ...
    if not days_either_side == 0:
        ...
        def dates_either_side_group_by(x):
            ...
            new_dates = [
                date + datetime.timedelta(int(value)) for value in new_date_values
            ]
            x = pd.concat(x)
            x["date"] = new_dates
            return x
        ...
        groupby_names = (
            ["patient_id", "date", "event", "agitation_label"]
            if return_event
            else ["patient_id", "date", "agitation_label"]
        )
        df_labels = (
            df_labels.groupby(groupby_names)
...
if __name__ == "__main__":
    ...
    labels = get_labels()
    print(
        "Positive Lables : {} - Negative Labels : {}".format(
            np.sum(labels["agitation_label"] == True),
            np.sum(labels["agitation_label"] == False),
        )
    )
    ...
    print(
        f"Agitation: There are {n_positives} positively and {n_negatives} negatively labelled rows."
    )
