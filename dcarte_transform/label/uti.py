...
def get_labels(days_either_side: int = 0, return_event: bool = False) -> pd.DataFrame:
    ...
    # accounting for different dcarte versions
    try:
        df_labels = dcarte.load("procedure", "raw")
    except Exception as e:
        df_labels = dcarte.load("procedure", "care")
    ...
    def uti_filter(x):
        ...
        type = str(x.type)
        notes = str(x.notes)
        if "urine" in type or "urinalysis" in type or "uti" in type:
            keep = True
            return keep
        elif "urine" in notes or "urinalysis" in notes or "uti" in notes:
            keep = True
            return keep
        return keep
...
if __name__ == "__main__":
    ...
    print(
        f"There are {n_positives} positively and {n_negatives} negatively labelled rows."
    )
