...
def datetime_rolling(
    ...
    while end_date < max_date:
        if dataframe_apply:
            values = df[
                df[datetime_col].between(start_date, end_date, inclusive="left")
            ]
        else:
            values = df[
                df[datetime_col].between(start_date, end_date, inclusive="left")
...
def datetime_compare_rolling(
    ...
    while end_date < max_date:
        ...
        all_window_values = df[
            df[datetime_col].between(start_date, end_date, inclusive="left")
        ]

        distribution_values = (
            all_window_values[
                all_window_values[datetime_col].between(
                    start_date,
        ...
        sample_values = (
            all_window_values[
                all_window_values[datetime_col].between(
                    distribution_end_date, end_date, inclusive="left"
                )
            ]
        )[value_col].values

        result_dict[datetime_col].append(
            end_date if label == "right" else distribution_end_date
        )
...
def groupby_freq(df, groupby_cols, count_col):
    groupby_cols = list(groupby_cols)
    groupby_cols.append(count_col)
    return df.groupby(by=groupby_cols).size().to_frame(name="freq")


def between_time(df, time_range, datetime_col):
    return df.pipe(
        lambda x: (
            x.set_index(datetime_col)
...
if __name__ == "__main__":
    ...
    pandarallel_progress(
        desc="Computing transition median deltas", smoothing=0, **tqdm_style
    )
    pandarallel_.initialize(progress_bar=True)
    ...
    daily_rel_transitions = (
        transitions[["patient_id", "transition", "start_date", "dur"]]
        .sort_values("start_date")
        .dropna()
        .groupby(
            by=[
    ...
    daily_rel_transitions["date"] = pd.to_datetime(
        daily_rel_transitions["start_date"]
    ).dt.date
    daily_rel_transitions = daily_rel_transitions.reset_index(drop=False).drop(
        ["level_2", "start_date"], axis=1
    )
